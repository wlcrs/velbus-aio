...
async def test_thermostat_operating_mode(status_mode, mode_name, sleep_timer):
    ...
    assert m.spec.temperature_channel is not None
    chan = m.map_channel_number(m.spec.temperature_channel)
    m.channels[chan] = Temperature(
    ...
    assert m.channels[chan].get_climate_mode() == mode_name
    assert m.channels[chan].sleep_timer == sleep_timer

    await m.channels[chan].set_climate_mode(mode_name)

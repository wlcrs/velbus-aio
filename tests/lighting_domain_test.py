...
@pytest.mark.asyncio
async def test_lighting_domain_relay(mock_controller):
...
@pytest.mark.asyncio
async def test_lighting_domain_relay3(mock_controller):
...
@pytest.mark.asyncio
async def test_lighting_domain_dimmer_and_dali(mock_controller):
    """Test routing of Dimmer and DALI dim values."""
    module = Module(1, 0x15, controller=mock_controller)
    ...
    await module.on_message(dali_msg)

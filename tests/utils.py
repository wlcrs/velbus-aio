...
from velbusaio.const import PRIORITY_FIRMWARE, PRIORITY_HIGH, PRIORITY_LOW, PRIORITY_THIRDPARTY

_PRIORITY_STR_MAP: dict[str, int] = {
    "low": PRIORITY_LOW,
    "high": PRIORITY_HIGH,
    "firmware": PRIORITY_FIRMWARE,
    "thirdparty": PRIORITY_THIRDPARTY,
}
...
def assert_roundtrip(
    ...
    *,
    ...
    if priority is not None:
        expected_priority = _PRIORITY_STR_MAP.get(priority, priority)
        actual_priority = _PRIORITY_STR_MAP.get(msg.priority, msg.priority)
        assert actual_priority == expected_priority
    elif getattr(msg, "_priority", None) is not None:
        expected_priority = _PRIORITY_STR_MAP.get(msg._priority, msg._priority)
        actual_priority = _PRIORITY_STR_MAP.get(msg.priority, msg.priority)
        assert actual_priority == expected_priority

...
@pytest.mark.asyncio
async def test_input_domain_push_button_status():
    """Test push button opened and closed status routing."""
    module = Module(1, 0x08)
    writer = AsyncMock()
    btn1 = Button(module, 1, "Button 1", False, True, writer, 1)
    btn2 = Button(module, 2, "Button 2", False, True, writer, 1)
...
@pytest.mark.asyncio
async def test_input_domain_counter():
    """Test counter pulses and power/energy routing."""
    module = Module(1, 0x08)
    writer = AsyncMock()
    counter = ButtonCounter(module, 1, "Counter 1", False, True, writer, 1)
    ...
    c_status.delay = 10
    await module.dispatch_message(c_status)
    ...
    await module.dispatch_message(c_val)
...
@pytest.mark.asyncio
async def test_input_domain_sensor_raw():
    """Test analog sensor raw value routing."""
    module = Module(1, 0x08)
    writer = AsyncMock()
    sensor = Sensor(module, 1, "Sensor 1", False, True, writer, 1)

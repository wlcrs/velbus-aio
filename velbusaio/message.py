...
class Message:
    ...
    def __init__(
        self,
        address: int | None = 0,
        priority: MessagePriority | int | None = None,
        rtr: bool | None = None,
        ...
        if address is None:
            address = 0
        elif not isinstance(address, int) or not (0 <= address <= 255):
            raise ValueError(
        ...
        if priority is None:
            priority = getattr(self, "_priority", MessagePriority.LOW)
        if isinstance(priority, str):
            priority_map = {
                "low": MessagePriority.LOW,
                "high": MessagePriority.HIGH,
                "firmware": MessagePriority.FIRMWARE,
                "thirdparty": MessagePriority.THIRDPARTY,
            }
            priority = priority_map.get(priority.lower(), MessagePriority.LOW)
        elif priority is None:
            priority = MessagePriority.LOW
        elif not isinstance(priority, MessagePriority):
            try:
    ...
    def set_attributes(self, priority: Any, address: int, rtr: bool) -> None:
        """Set message routing attributes."""
        if isinstance(priority, str):
            priority_map = {
                "low": MessagePriority.LOW,
                "high": MessagePriority.HIGH,
            ...
            self.priority = priority_map.get(priority.lower(), MessagePriority.LOW)
        elif isinstance(priority, MessagePriority):
    ...
    def populate(
        ...
        """Populate attributes from wire packet."""
    ...
    def from_bytes(
        ...
        msg = cls(address=address, priority=priority, rtr=rtr)
    ...
    def __repr__(self) -> str:
        data_str = binascii.hexlify(self.data, " ") if isinstance(self.data, (bytes, bytearray)) else repr(self.data)
        return (
            f"{self.__class__.__name__}(priority={int(self.priority):02x}, address={self.address:02x},"
...
def _parse_raw_frame(
    ...
    if len(rawmessage) < MINIMUM_MESSAGE_SIZE or len(rawmessage) > MAXIMUM_MESSAGE_SIZE:
        raise ValueError("Received a raw message with an illegal length")

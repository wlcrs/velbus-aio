...
async def handle_meteo_raw(module: Module, message: MeteoRawMessage) -> None:
    ...
    if rain_ch := sensors.get(11):
        ...
        _LOG.warning(
            f"Received meteo rain data for missing channel 11 on module {module.get_address()}"
        )
    ...
    if light_ch := sensors.get(12):
        ...
        _LOG.warning(
            f"Received meteo light data for missing channel 12 on module {module.get_address()}"
        )
    ...
    if wind_ch := sensors.get(13):
        ...
        _LOG.warning(
            f"Received meteo wind data for missing channel 13 on module {module.get_address()}"
        )
